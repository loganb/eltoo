...
src_configure() {
	...
	RELEASE=T ./configure --skip-prebuilt \
		--prefix ${EPREFIX}/usr \
...
src_install() {
	...
	#dosym "/${librbx}/${minor_version}/bin/rbx" "#{EPREFIX}/usr/bin/rbx" || die "Couldn't make rbx symlink"

	insinto /${librbx}/${minor_version}/site
	doins "${FILESDIR}/auto_gem.rb" || die "Couldn't install rbx auto_gem.rb"
	RBX_RUNTIME="${S}/runtime" RBX_LIB="${S}/lib" bin/rbx compile "${ED}/${librbx}/${minor_version}/site/auto_gem.rb" || die "Couldn't bytecompile auto_gem.rb"
}
